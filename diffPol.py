...
class DiffPolicy(nn.Module):
    ...
    def inference(self,
        ...
        pred_actions = noisy_actions.clone()
        if diffusion_iters is None:
            diffusion_iters = self.num_diffusion_iters
    ...
    def forward(self,
        ...
        timesteps = torch.randint(
            ...
            (b_size,), device=self.device
        ).long()
        # print(f"timesetps: {timesteps.shape}")
        ...
        noise_pred = self.noise_pred_net(
            noisy_actions, timesteps, global_cond=obs_enc)
